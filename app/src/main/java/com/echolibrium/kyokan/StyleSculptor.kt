...
object StyleSculptor {
    ...
    // Calibrated breath direction vector — computed from real voices.
    // When null, applyBreathiness() falls back to the heuristic approximation.
    @Volatile
    private var calibratedBreathDirection: FloatArray? = null
    ...
    private fun applyBreathiness(style: FloatArray, breathIntensity: Int) {
        ...
        val breathDir = calibratedBreathDirection
        if (breathDir != null) {
            // Use calibrated direction from real voice data (clear → breathy)
            for (i in style.indices) {
                style[i] += breathDir[i] * intensity
            }
        } else {
            // Heuristic fallback — alternating ±direction by dimension index
            for (i in style.indices) {
    ...
    /**
     * Calibrate the breath direction from real voice data.
     *
     * Computes the normalized direction vector from a clear voice (af_bella, sid=1)
     * to a breathy voice (af_sky, sid=4) and stores it for use in applyBreathiness().
     *
     * Call once after DirectOrtEngine initialization. If either voice style lookup
     * fails, the calibration is skipped and the heuristic fallback remains active.
     *
     * @param engine DirectOrtEngine with loaded voices.bin
     */
    fun calibrateBreathDirection(engine: DirectOrtEngine) {
        val numTokens = 50  // representative utterance length
        val bellaStyle = engine.getKokoroStyle(voiceIndex = 1, numTokens = numTokens)  // af_bella (clear)
        val skyStyle   = engine.getKokoroStyle(voiceIndex = 4, numTokens = numTokens)  // af_sky (breathy)

        if (bellaStyle == null || skyStyle == null) return

        calibratedBreathDirection = computeBreathDirection(bellaStyle, skyStyle)
    }
